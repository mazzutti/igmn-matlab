function [x, fval, exitFlag] = imode(optOptions, lb, ub, num_vars, varargin) %#codegen
    hpNames = {[repmat('phi', num_vars, 1), num2str((1:num_vars)')]};

    lbRow = repmat (lb, 1, num_vars);
    ubRow = repmat (ub, 1, num_vars);
    
    % Start the core algorithm
    [x, fval, exitFlag] = imodecore(num_vars, lbRow, ubRow, optOptions, hpNames, 1:num_vars, varargin{:});
end

function [x,fval,exitFlag] = imodecore(nhps, lbRow, ubRow, optOptions, hpNames, hpIndexes, varargin)
    ...
    objFun = @(params) evaluate(params, varargin{:});
    ...
    if  optOptions.Verbosity > 1
        ...
        meanFvalDisplay = log(1 + mean(state.Fvals, 'all'));
        fprintf('\n                                 Best            Log(Mean)     Stall\n');
    ...
    function optimValues = i_updateOptimValues
        ...
        optimValues.meanfval = mean(state.Fvals);
