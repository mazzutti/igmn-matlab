function [x, fval, exitFlag] = pso(optOptions, lb, ub, num_vars, hpNames, targets, Phi) %#codegen
    % lbRow = repmat (lb, 1, num_vars);
    % ubRow = repmat (ub, 1, num_vars);
    
    % Start the core algorithm
    [x, fval, exitFlag] = psocore(num_vars, lb, ub, optOptions, hpNames, 1:num_vars, targets, Phi);
end


function [x,fval,exitFlag] = psocore(nhps, lbRow, ubRow, optOptions, hpNames, hpIndexes, targets, Phi) %#codegen
    ...
    objFun = @(params) evaluate(params, targets, Phi);
