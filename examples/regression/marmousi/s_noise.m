...
if ~isempty(param.ormsby)
   ...
   temp=s_filter(s_convert(temp,0, 9028), ...
        {'ormsby',param.ormsby});
   temp=temp.traces;
end
...
if strcmpi(param.output,'seismic')
   seismic=seismic+(param.ratio*amp/ampn)*temp;
   seismic=s_history(seismic,'append',htext);

elseif strcmpi(param.output,'noise')
   seismic=s_convert((param.ratio*amp/ampn)*temp,0,4, ...
   htext,'ms');
